...
def predict_sequence(
    ...
    if confidence < confidence_threshold:
        return "incerto", confidence

    label = idx_to_class.get(pred_idx, f"classe_{pred_idx}")
...
def main() -> None:
    ...
    capturing = False
    capture_buffer: deque[np.ndarray] = deque(maxlen=sequence_length)
    predicted_label = "nenhuma"
    predicted_confidence = 0.0
    status_text = "Pressione ESPACO para capturar"
    ...
    try:
        while True:
            ...
            if capturing:
                capture_buffer.append(flattened)
                status_text = f"Capturando: {len(capture_buffer)}/{sequence_length}"

                if len(capture_buffer) == sequence_length:
                    sequence = np.array(capture_buffer, dtype=np.float32)
                    predicted_label, predicted_confidence = predict_sequence(
                        model=model,
                        sequence=sequence,
                    ...
                    status_text = "Captura concluida. Pressione ESPACO para nova captura"

            text_1 = f"Predicao: {predicted_label}"
            text_2 = f"Confianca: {predicted_confidence:.2%}"
            text_3 = status_text
            text_4 = "ESPACO = capturar gesto | ESC = sair"
            ...
            if key == 32 and not capturing:  # ESPACO
                capturing = True
                capture_buffer.clear()
                predicted_label = "capturando..."
                predicted_confidence = 0.0
                status_text = "Iniciando captura"
